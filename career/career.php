...
// ตรวจสอบ session riot_id
if (!isset($_SESSION['riot_id'])) {
    header("Location: login.php");
    exit;
}

$full_id = $_SESSION['riot_id'];

// ดึง region จากฐานข้อมูล
$stmt = $conn->prepare("SELECT region FROM users WHERE riot_id = ?");
...
if ($result->num_rows > 0) {
    ...
    $update_stmt = $conn->prepare("UPDATE users SET region = ? WHERE riot_id = ?");
    $update_stmt->bind_param("ss", $region, $full_id);
    $update_stmt->execute();
    $update_stmt->close();
}
$stmt->close();
...
// Add error logging
if (empty($region)) {
    error_log("Region lookup failed for riot_id: " . $full_id);
...
if ($match_data && isset($match_data['data'])) {
    ...
    $paginated_matches = array_slice($match_data['data'], 0, 300);
}
?>
...
<head>
    ...
    <title><?= htmlspecialchars($full_id) . ' Player Info' ?></title>
    ...
    <div class="container">
        ...
        <div class="mt-5">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4>Match History (Showing <?= count($paginated_matches) ?> of <?= min(300, $match_data['total_available']) ?> matches)</h4>
                <form id="filter-form" method="GET" class="d-flex">
                    <!-- ไม่ต้องส่ง riot_id/region เพราะใช้จาก session -->
